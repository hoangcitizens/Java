...
/*
    ...
    địa chỉ, tuổi, số điện thoại, mức lương, số năm kinh nghiệm. Trong đó địa chỉ nhân viên là 1 trong 63 tỉnh thành phố
    của Việt Nam biểu diễn bởi một enum. Thiết kế lớp mô tả đối tượng nhân viên như trên, sử dụng tính đóng gói dữ liệu
    và bổ sung các phương thức phụ trợ cần thiết. Tạo các constructor đầy đủ và tái sử dụng sao cho hợp lý. Lập danh
    sách(dùng mảng) các nhân viên sau đó tạo menu cho phép thực hiện các chức năng cho tới khi nào người dùng muốn kết
    thúc thì dừng chương trình:
...
// todo 4: sử dụng enum sao cho phù hợp
public class Test {
            ...
            switch (choice) {
                case 1 -> {
                    if (employeeCount <= EMP_NUM_MAX) {
                        var employee = createNewEmployee(employees, input);
                        if (employee == null) {
                ...
                case 10 -> System.out.println("==> Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi! <==");
                default -> System.out.println("==> Chọn sai chức năng. Vui lòng chọn số từ 1 -> 10.");
            }
        } while (choice != 10);
    }
    ...
    private static int createFakeEmployee(Employee[] employees) {
        var counter = 0;
        employees[counter++] = new Employee("EMP1001", "Lê Hoàng Nam", Address.HA_NOI, 25, "0123456789", 16500000, 5);
        employees[counter++] = new Employee("EMP1002", "Nguyễn Minh Tuấn", Address.HO_CHI_MINH, 27, "0123456749", 18500000, 7);
        employees[counter++] = new Employee("EMP1003", "Nguyễn Thảo Ngân", Address.BINH_DINH, 21, "0123456779", 14500000, 4);
        employees[counter++] = new Employee("EMP1004", "Trần Đình Long", Address.THANH_HOA, 24, "0123456719", 12300000, 3);
        employees[counter++] = new Employee("EMP1005", "Hoàng Văn Minh", Address.NGHE_AN, 26, "0123456759", 17500000, 6);
        employees[counter++] = new Employee("EMP1006", "Lưu Thị Hà", Address.DA_NANG, 20, "0123456769", 10500000, 2);
        return counter;
    }
    ...
    private static Employee createNewEmployee(Employee[] employees, Scanner input) {
        System.out.print("Mã nhân viên(EMPxxxx): ");
        String id = input.nextLine().trim().toUpperCase();
        if (id.startsWith("EMP") && id.length() == 7 && isUniqueId(employees, id)) {
    ...
    private static boolean isUniqueId(Employee[] employees, String id) {
        for (Employee employee : employees) {
