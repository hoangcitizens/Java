...
public class Test {
    ...
    public static void main(String[] args) {
        ...
        // todo 4: tạo mảng các sản phẩm với số lượng tối đa = 100
        Item[] items = new Item[100];
        var itemCount = createFakeItems(items);

        // todo 5: triển khai menu chứa các chức năng của chương trình
        int choice;
        do {
            displayMenu();
            ...
            switch (choice) {
                // todo 8: triển khai code tương ứng với chức năng số 1
                case 1:
                    if (itemCount <= 100) {
                    ...
                    break;
                // todo 13: triển khai code tương ứng với chức năng số 2
                case 2:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 15: triển khai code tương ứng với chức năng số 3
                case 3:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 17: triển khai code tương ứng với chức năng số 4
                case 4:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 19: triển khai code tương ứng với chức năng số 5
                case 5:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 21: triển khai code tương ứng với chức năng số 6
                case 6:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 23: triển khai code tương ứng với chức năng số 7
                case 7:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 25: triển khai code tương ứng với chức năng số 8
                case 8:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 27: triển khai code tương ứng với chức năng số 9
                case 9:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 29: triển khai code tương ứng với chức năng số 10
                case 10:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 31: triển khai code tương ứng với chức năng số 11
                case 11:
                    if (itemCount > 0) {
                        statRevenue(items);
                    } else {
                    ...
                    break;
                // todo 34: triển khai code tương ứng với chức năng số 12
                case 12:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 36: triển khai code tương ứng với chức năng số 13
                case 13:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 39: triển khai code tương ứng với chức năng số 14
                case 14:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 41: triển khai code tương ứng với chức năng số 15
                case 15:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 44: triển khai code tương ứng với chức năng số 16
                case 16:
                    if (itemCount > 0) {
                    ...
                    break;
                // todo 47: triển khai code tương ứng với chức năng số 17
                case 17:
                    System.out.println("==> Cảm ơn bạn đã sử dụng dịch vụ của chúng tôi! <==");
                    break;
                // todo 48: triển khai code tương ứng với chức năng không tồn tại
                default:
                    System.out.println("==> Sai chức năng, vui lòng nhập từ 1 -> 17 <==");
            }
        } while (choice != 17);
    }
    ...
    // todo 6: triển khai phương thức hiển thị danh sách các lựa chọn
    ...
    // todo 9: triển khai phương thức tạo mới một sản phẩm
    private static Item createItem(Item[] items, Scanner input) {
        ...
        if (index != -1) { // nếu id của item đã tồn tại
            ...
            items[index].inventoryQuantity += Integer.parseInt(input.nextLine());
            return null;
        }
    ...
    // todo 12: triển khai code kiểm tra tính hợp lệ của ngày sản xuất và ngày hết hạn
    ...
    // todo 10: viết phương thức kiểm tra tính hợp lệ của mã sản phẩm
    private static boolean checkItemId(String id) {
        return id.toUpperCase().startsWith("ITM") && id.length() == 7;
    }

    // todo 11: triển khai code kiểm tra và trả về chỉ số phần tử của sản phẩm trong mảng
    private static int findItemById(Item[] items, String id) {
        for (int i = 0; i < items.length; i++) {
            if (items[i] == null) {
                break; // nếu không tìm thấy =>
            }
            if (items[i].id.compareToIgnoreCase(id) == 0) {
                return i;
            }
        }
    ...
    // todo 14: triển khai phương thức xoá một sản phẩm theo mã sản phẩm
    ...
    // todo 16: triển khai phương thức cập nhật giá một sản phẩm theo mã sản phẩm
    ...
    // todo 18: triển khai phương thức cập nhật ngày sản xuất và hết hạn cho một sản phẩm theo mã sản phẩm
    private static void updateItemDate(Item[] items, Scanner input) {
        ...
        if (index != -1) {
            ...
            if (validateDate(newProductionDate)) { // nếu ngày hợp lệ
                items[index].productionDate = newProductionDate; // sử dụng
            } else { // ngày không hợp lệ => lấy ngày mặc định
                items[index].productionDate = "01/01/2024";
            }
            if (validateDate(newProductionDate)) {
                items[index].expirationDate = newExpirationDate;
            } else {
                items[index].expirationDate = "01/06/2024";
            }
    ...
    // todo 20: triển khai phương thức cập nhật số hàng tồn kho cho sản phẩm theo mã sản phẩm
    private static void updateItemQuantity(Item[] items, Scanner input) {
        ...
        if (index != -1) {
            ...
            if (newQuantity > 0) {
                items[index].inventoryQuantity = newQuantity;
    ...
    // todo 22: triển khai phương thức tính tổng tiền hàng tồn kho trong cửa hàng
    private static float calculateTotalItemPrice(Item[] items) {
        ...
        for (Item item : items) {
            ...
            totalMoney += item.price * item.inventoryQuantity;
        }
        return totalMoney;
    }

    // todo 24: triển khai phương thức hiển thị danh sách các mặt hàng hiện có
    private static void displayAllItems(Item[] items) {
        ...
        for (Item item : items) {
            ...
            System.out.printf("%-10s%-25s%-20s%-15s%-12s%-12s%-10s%-10s\n",
                    item.id, item.name, item.manufacturer, decimalFormat.format(item.price),
                    item.productionDate, item.expirationDate, item.inventoryQuantity, item.soldQuantity);
        }
    }

    // todo 33: triển khai phương thức hiển thị danh sách mặt hàng và doanh thu
    private static void showRevenue(Item[] items) {
        ...
        for (Item item : items) {
            ...
            var revenue = item.soldQuantity * item.price;
            System.out.printf("%-10s%-25s%-20s%-15s%-10s%-10s%-15s\n",
                    item.id, item.name, item.manufacturer, decimalFormat.format(item.price),
                    item.inventoryQuantity, item.soldQuantity, decimalFormat.format(revenue));
        }
    }

    // todo 26: triển khai phương thức sắp xếp mặt hàng theo thứ tự giá giảm dần
    ...
    // todo 28: triển khai phương thức sắp xếp mặt hàng theo thứ tự giá tăng dần
    ...
    // todo 30: triển khai phương thức bán sản phẩm
    private static void sellItem(Scanner input, Item[] items) {
        ...
        if (index != -1) {
            ...
            System.out.println(STR."Bạn chỉ có thể bán tối đa: \{soldItem.inventoryQuantity} sản phẩm");
            System.out.println("Số lượng muốn bán: ");
            int soldAmount = Integer.parseInt(input.nextLine());
            if (soldAmount >= 1 && soldAmount <= soldItem.inventoryQuantity) {
                soldItem.soldQuantity += soldAmount;
                soldItem.inventoryQuantity -= soldAmount;
    ...
    // todo 32: triển khai phương thức tính tổng tiền hàng đã bán trong cửa hàng
    private static void statRevenue(Item[] items) {
        ...
        for (Item item : items) {
            ...
            var revenue = item.price * item.soldQuantity;
            totalRevenue += revenue;
            totalRemain += item.inventoryQuantity;
            totalSoldItem += item.soldQuantity;
        }
    ...
    // todo 35: triển khai phương thức tìm kiếm tất cả các sản phẩm bán được hàng
    ...
    // todo 37: triển khai phương thức tìm kiếm các sản phẩm bán được nhiều nhất
    private static Item[] findMostSoldItems(Item[] items) {
        ...
        int soldItemCount = 0;
        for (var item : items) {
            if (item != null && item.soldQuantity == maxSoldAmount) {
    ...
    // todo 38: triển khai code tìm số lượng bán nhiều nhất trong danh sách
    private static int findMaxItemSoldQuantity(Item[] items) {
        int max = Integer.MIN_VALUE;
        for (Item item : items) {
            if (item != null && item.soldQuantity > max) {
    ...
    // todo 40: triển khai phương thức tìm kiếm tất cả các sản phẩm bị ế
    private static Item[] findAllNotSoldItems(Item[] items) {
        var results = new Item[items.length];
        int soldItemCount = 0;
        for (Item item : items) {
            if (item != null && item.soldQuantity == 0) {
    ...
    // todo 42: triển khai phương thức tìm kiếm tất cả các sản phẩm đã hết hạn tính đến thời điểm hôm nay
    private static Item[] findAllItemsExpired(Item[] items) {
        ...
        for (Item item : items) {
            if (item != null && isExpired(currentDate, item.expirationDate)) {
    ...
    // todo 43: triển khai code kiểm tra xem một sản phẩm đã hết hạn sử dụng chưa
    ...
    // todo 45: triển khai phương thức tìm kiếm tất cả các sản phẩm sắp hết hạn trước đó 3 ngày so vs thời gian hiện tại
    private static Item[] findAllItemsExpiredSoon(Item[] items) {
        ...
        for (Item item : items) {
            if (item != null && isExpiredSoon(item.expirationDate, currentDate)) {
                expiredItems[expiredItemCount++] = item;
            }
        }
    ...
    // todo 45: triển khai phương thức kiểm tra xem một sản phẩm sắp hết hạn sử dụng chưa
