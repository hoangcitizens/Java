...
/*
    Bài 6. Thông tin về môn học bao gồm: mã môn học có dạng SJxxxx(ví dụ SJ1005), phần số tự động tăng từ 1000,
    tên môn học, số tín chỉ, số tiết học, số lượng bài kiểm tra, số bài thực hành, số lượng bài tập lớn, loại môn học.
    Loại môn học bao gồm: đại cương, cơ sở ngành, chuyên ngành, tự chọn, bắt buộc. Bổ sung các constructor và tái sử
    dụng chúng sao cho phù hợp, sử dụng tính đóng gói dữ liệu và tự bổ sung các phương thức phụ trợ cần thiết. Viết
    chương trình có menu chọn thực hiện các chức năng sau:
        1. Thêm mới môn học vào danh sách. Mỗi môn học xuất hiện không quá 1 lần trong danh sách này.
...
public class Test {
    ...
    private static int createFakeSubjects(Subject[] subjects) {
        var index = 0;
        subjects[index++] = new Subject(null, "Đại số", 3, 36, 3, 0, 1, Type.GENERAL);
        subjects[index++] = new Subject(null, "Vật lý", 4, 48, 4, 4, 1, Type.GENERAL);
        subjects[index++] = new Subject(null, "Giải tích", 3, 36, 3, 0, 1, Type.GENERAL);
        subjects[index++] = new Subject(null, "Lập trình C++", 3, 36, 3, 4, 1, Type.MAJOR_FOUNDATION);
        subjects[index++] = new Subject(null, "Lập trình Java", 4, 48, 4, 4, 2, Type.MAJOR_FOUNDATION);
        subjects[index++] = new Subject(null, "SQL", 3, 36, 3, 4, 1, Type.MAJOR_FOUNDATION);
        subjects[index++] = new Subject(null, "Lập trình Java web", 5, 60, 5, 5, 2, Type.MAJOR_SPECIALIZED);
        subjects[index++] = new Subject(null, "Lập trình Android", 5, 60, 5, 5, 2, Type.MAJOR_SPECIALIZED);
        return index;
    }
    ...
    private static void sortSubjectsByNameASC(Subject[] subjects, int size) {
        for (int i = 0; i < size - 1; i++) {
            for (int j = size - 1; j > i; j--) {
                if (subjects[j - 1].getName().compareToIgnoreCase(subjects[j].getName()) > 0) {
                    var temp = subjects[j - 1];
